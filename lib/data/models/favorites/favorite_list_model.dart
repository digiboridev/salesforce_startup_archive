import 'dart:convert';
import 'package:salesforce.startup/data/models/favorites/favorite_item_model.dart';
...
class FavoriteListModel extends FavoriteList {
  ...
  }) : super(sFId: sFId, listName: listName, favoriteItems: favoriteItems, isAllList: isAllList);
  ...
  factory FavoriteListModel.fromMap(Map<String, dynamic> map) {
    return FavoriteListModel(
      ...
      favoriteItems: List<FavoriteItemModel>.from(map['FavoriteItems']?.map((x) => FavoriteItemModel.fromMap(x)) ?? const []),
    );
  }
  ...
  factory FavoriteListModel.fromEntity(FavoriteList favoriteList) {
    return FavoriteListModel(
      ...
      favoriteItems: favoriteList.favoriteItems.map((e) => FavoriteItemModel.fromEntity(e)).toList(),
    );
  }

  String toJson() => json.encode(toMap());

  factory FavoriteListModel.fromJson(String source) => FavoriteListModel.fromMap(json.decode(source));
}

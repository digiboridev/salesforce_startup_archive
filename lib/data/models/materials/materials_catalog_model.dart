...
class MaterialsCatalogModel extends MaterialsCatalog {
  ...
  }) : super(materials: materials, families: families, classifications: classifications, brands: brands, hierarchys: hierarchys);
  ...
  factory MaterialsCatalogModel.fromMap(Map<String, dynamic> map) {
    return MaterialsCatalogModel(
      materials: List<MaterialModel>.from(map['materials']?.map((x) => MaterialModel.fromMap(x)) ?? const []),
      families: List<FamilyModel>.from(map['families']?.map((x) => FamilyModel.fromMap(x)) ?? const []),
      classifications: List<ClassificationModel>.from(map['classifications']?.map((x) => ClassificationModel.fromMap(x)) ?? const []),
      brands: List<BrandModel>.from(map['brands']?.map((x) => BrandModel.fromMap(x)) ?? const []),
      hierarchys: List<HierarchyModel>.from(map['hierarchys']?.map((x) => HierarchyModel.fromMap(x)) ?? const []),
    );
  }

  factory MaterialsCatalogModel.fromEntity(MaterialsCatalog materialsCatalog) {
    return MaterialsCatalogModel(
      materials: materialsCatalog.materials.map((e) => MaterialModel.fromEntity(e)).toList(),
      families: materialsCatalog.families.map((e) => FamilyModel.fromEntity(e)).toList(),
      classifications: materialsCatalog.classifications.map((e) => ClassificationModel.fromEntity(e)).toList(),
      brands: materialsCatalog.brands.map((e) => BrandModel.fromEntity(e)).toList(),
      hierarchys: materialsCatalog.hierarchys.map((e) => HierarchyModel.fromEntity(e)).toList(),
    );
  }

  String toJson() => json.encode(toMap());

  factory MaterialsCatalogModel.fromJson(String source) => MaterialsCatalogModel.fromMap(json.decode(source));
}

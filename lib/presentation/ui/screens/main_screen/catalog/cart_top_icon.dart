...
class CartTopIconState extends State<CartTopIcon>{
  ...
  Widget build(BuildContext context) {
    switch (type) {
      ...
      case CartTopIcon.menu_type:
        return getMenu();
      default: return getFavorite();
    }

  }

  Widget getFavorite(){
    return Padding(padding: EdgeInsets.only(left: Get.width*0.02, right: Get.width*0.02),
    child: Image.asset("assets/icons/favorite.png", ));
  }
  Widget getFavoriteSelect(){
    return Padding(padding: EdgeInsets.only(left: Get.width*0.02, right: Get.width*0.02),
        child: Image.asset("assets/icons/favorite_select.png", ));
  }
  ...
  Widget getMenu(){
    return Container(
      child: Icon(Icons.more_vert,color: MyColors.gray_B5C0CD,),);
  }

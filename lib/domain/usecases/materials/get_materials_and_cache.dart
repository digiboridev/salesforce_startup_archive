...
class GetMaterialsAndCache implements UseCase<MaterialsCatalog, GetMaterialsAndCacheParams> {
  ...
  Future<MaterialsCatalog> call(GetMaterialsAndCacheParams params) async {
    try {
      MaterialsCatalog cachedMaterialsCatalog = await _loadCache(params: params);
      return cachedMaterialsCatalog;
    } catch (e) {
      if (!params.hasConnection) {
        throw InternalException('No connection');
      }
      MaterialsCatalog remoteMaterialsCatalog = await materialsRepository.getRemoteCatalog(customerSAP: params.customerSAP);

      materialsRepository.setLocalMaterials(customerSAP: params.customerSAP, materialsCatalog: remoteMaterialsCatalog);

      materialsRepository.setMaterialsSyncData(customerSAP: params.customerSAP, syncData: SyncData(syncDateTime: DateTime.now(), locale: params.locale));
      return remoteMaterialsCatalog;
    }
  }

  Future<MaterialsCatalog> _loadCache({required GetMaterialsAndCacheParams params}) async {
    if (await _cacheValid(params: params)) {
      return materialsRepository.getLocalMaterials(customerSAP: params.customerSAP);
    } else {
      throw InternalException('Unable to load catalog for consumer ${params.customerSAP}');
    }
  }

  Future<bool> _cacheValid({required GetMaterialsAndCacheParams params}) async {
    try {
      SyncData syncData = await materialsRepository.getMaterialsSyncData(customerSAP: params.customerSAP);
